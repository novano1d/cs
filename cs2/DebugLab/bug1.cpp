...
int main() {
    ...
    //if (inArray(ar, 3, 3)) std::cout << "this works";
    srand(9809);  //Seed random number generator.

    for (int i = 0; i < REP; i++) {
        //std::cerr << i << std::endl; //only prints 0 which means infinite loop is in the method call
...
void UniqRandInt(int max, int n, int result[]) {
    ...
    while(cntr < n) {
        //std::cerr << cntr << std::endl; //infinitely prints 0s meaning that cntr is never actually incremented
        r = rand();  //Get random number
        r = r % (max + 1);
        //std::cerr << r << std::endl;
        if (!inArray(result, n, r)) { //this statement never returns true (until I fixed it) b/c when nothing is in the array it would cause an infinite loop
            //std::cerr << "in if";
            result[cntr] =  r;
            cntr++;
        }
