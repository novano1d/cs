...
AST::~AST() {
    ...
    std::list<AST*>::iterator pointer = child.begin();
    while (pointer != child.end())
    {
        delete *pointer;
        pointer++;
    }
...
AST::AST(const AST& actual) {
    ...
    //make a copy of each node putting its
    //into this.
    tag = actual.tag;
    closeTag = actual.closeTag;
    nodeType = actual.nodeType;
    text = actual.text;
    std::list<AST*>::const_iterator pointer = actual.child.begin();
    while (pointer != actual.child.end())
    {
        child.push_back(new AST(**pointer));
        pointer++;
    }
}
...
void AST::mainHeader(const std::vector<std::string>& profileName,
    ...
    auto it = std::find(child.begin(), child.end(), getChild("function"));
    child.insert(it, new AST(token, "#include \"profile.hpp\"\n"));
    for (int i = 0; i < profileName.size(); i++) child.insert(it, new AST(token, "profile " + profileName[i] + "(\"" + fileName[i] + "\");\n")); //the two arrays should be parallel
}
...
void AST::mainReport(const std::vector<std::string>& profileName) {
    ...
    {
        AST* current = *it;
        if (current->tag != "function") continue;
        AST* name = current->getChild("name");
        if (name->getName() == "main") break;
        it++;
    }
    AST* result = *it;
    AST* block = result->getChild("block")->getChild("block_content");
    it = block->child.end();
    it--; 
    it--;
    for (int i = 0; i < profileName.size(); i++)
    {
        block->child.insert(it, new AST(token, "std::cout << " + profileName[i] + " << std::endl;\n"));
    }
}
...
void AST::functionCount(const std::string& profileName) {
    ...
    {
        AST* current = *it;
        if (current->tag != "function") {
            if (current->tag != "constructor") continue;
        }
        AST* block = current->getChild("block")->getChild("block_content");
        std::list<AST*>::iterator in = block->child.begin();
        in++;
        block->child.insert(in, new AST(token, profileName + ".count(__LINE__, \"" + current->getChild("name")->getName() + "\");\n"));
    }
...
void AST::lineCount(const std::string& profileName) {
    ...
    {
        AST* current = *i;
        if (current->nodeType == category)
        {
            if(current->tag == "expr_stmt")
            {
                child.insert(++i, new AST(token, profileName + ".count(__LINE__);\n"));
            }
            //challenge
            else if (current->tag == "condition") 
            {
                AST* expression = current->getChild("expr");
                auto begin = expression->child.begin();
                expression->child.insert(begin, new AST(token, profileName + ".count(__LINE__), "));
            }
            else
            {
                if(!isStopTag(current->tag)) current->lineCount(profileName);
            }
        }
    }
}
