...
template <typename Key, typename Value, typename Compare, typename Hash>
   void hashmap<Key, Value, Compare, Hash>::rehash(size_t n)
   {
      ...
      vector<Element> allPairs;
      for (int i = 0; i < elems_.size(); i++)
      {
         auto it = elems_[i].begin();
         while (it != elems_[i].end())
         {
...
pair<typename hashmap<Key, Value, Compare, Hash>::Element*, bool> hashmap<Key, Value, Compare, Hash>::insert(const Element& x) {
   ...
   return make_pair(find(x.first), it == elems_[bucket].end());
}
...
pair<typename hashmap<Key, Value, Compare, Hash>::Element*, bool> hashmap<Key, Value, Compare, Hash>::erase(const Key& x) {
   ...
   if (it != elems_[bucket].end()) {    // the element exists, erase it
      if (elems_[bucket].size() > 1)
      {
         outit = elems_[bucket].erase(it);
      }
      else elems_[bucket].erase(it);
      --size_;
   }
   else out = false;
   if (elems_[bucket].empty()) //if there isn't a next element in the bucket, search the next buckets until an element is found
   { //I wasn't sure if we were supposed to return the next element in terms of size, or next element in terms of down the hashmap. But I think this makes most sense in context.
      for (int i = bucket; i < elems_.size(); i++)
      {
         if (elems_[i].empty() && i == elems_.size()-1) return make_pair(nullptr, out);
         if (elems_[i].empty()) continue;
         outit = elems_[i].begin();
         break;
      }
   }
   return make_pair(&(*outit), out);
}
...
Value& hashmap<Key, Value, Compare, Hash>::operator[] (const Key& x) {
   ...
   if (found == nullptr) { // if key not found, create new element with empty value
      found = (insert(make_pair(x, Value())).first); // calling default constructor on Value
   }
   return found->second;
}